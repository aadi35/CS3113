...
void StartScreen::initialise()
{
   ...
   */
   std::vector<int> used;
   mGameState.boxes = new Entity[20];
   for (int i = 0; i < 20; i++){
   ...
   startscreen = LoadTexture("assets/game/Start.png");
...
//    for (int i = 0; i < 20; i++) mGameState.boxes[i].update(deltaTime, nullptr, mGameState.map, nullptr, 0);
...
void StartScreen::render()
{
   ...
   Rectangle texture = {0.0f, 0.0f ,static_cast<float>(startscreen.width), static_cast<float>(startscreen.height)};
   Rectangle screen = {0.0f, 0.0f , 1000.0f, 600.0f};
   DrawTexturePro(startscreen, texture, screen, {0,0}, 0.0f, WHITE);

//    mGameState.playerOne->render();
//    mGameState.playerTwo->render();
//    mGameState.map->render();
//   for (int i = 0; i < 20; i++) mGameState.boxes[i].render();
