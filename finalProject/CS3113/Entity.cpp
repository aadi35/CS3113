...
Entity::~Entity() { if (mTexture.id != 0) { 
    ...
    Entity *collidableEntities, int collisionCheckCount)
{
    ...
    Vector2 leftCentreProbe   = { mPosition.x - 25, mPosition.y };

    Vector2 rightCentreProbe  = { mPosition.x + 25, mPosition.y };

    Vector2 topCentreProbe    = { mPosition.x, mPosition.y - 25 };

    Vector2 bottomCentreProbe = { mPosition.x, mPosition.y + 25 };

    if (mDirection == UP && not map->isSolidTileAt(topCentreProbe, &xOverlap, &yOverlap) && moving) mPosition.y -= 25;
    if (mDirection == DOWN && not map->isSolidTileAt(bottomCentreProbe, &xOverlap, &yOverlap) && moving) mPosition.y += 25;
    if (mDirection == RIGHT && not map->isSolidTileAt(rightCentreProbe, &xOverlap, &yOverlap) && moving) mPosition.x += 25;
    if (mDirection == LEFT && not map->isSolidTileAt(leftCentreProbe, &xOverlap, &yOverlap) && moving) mPosition.x -= 25;
    ...
    mAngle += mAngularMovement * deltaTime;
    if (mEntityType == BLOCK) mAngle = 0;
}

void Entity::update(float deltaTime, Entity *playerOne, Map *map, 
    Entity *playerTwo)
{
    if (mEntityStatus == INACTIVE) return;
    
    if (mEntityType == NPC) AIActivate(nullptr);
