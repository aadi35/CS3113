...
enum BulletStatus { BACTIVE, BINACTIVE                   };
...
private:
    ...
    BulletStatus mBulletStatus = BACTIVE;
...
public:
    ...
    Bullet();
    Bullet(Vector2 position, Vector2 scale, const char *textureFilepath);
    Bullet(Vector2 position, Vector2 scale, const char *textureFilepath, 
        TextureType textureType, Vector2 spriteSheetDimensions, 
        std::map<Direction, std::vector<int>> animationAtlas);
    ~Bullet();

    void update(float deltaTime, Entity *playerOne, Entity *playerTwo, Map *map, 
        Entity *collidableEntities, int collisionCheckCount);
    void render();
    void normaliseMovement() { Normalise(&mMovement); }

    void jump()       { mIsJumping = true;  }
    void activate()   { mBulletStatus  = BACTIVE;   }
    void deactivate() { mBulletStatus  = BINACTIVE; }
    void displayCollider();

    bool isActive() { return mBulletStatus == BACTIVE ? true : false; }
