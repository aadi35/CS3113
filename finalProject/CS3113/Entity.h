...
public:
    ...
    void update(float deltaTime, Entity *playerOne, Map *map, 
        Entity *playerTwo);
    void render();
    ...
    void moveUp()    { mDirection = UP; moving = true;    }
    void moveDown()  { mDirection = DOWN; moving = true;  }
    void moveLeft()  { mDirection = LEFT; moving = true;  }
    void moveRight() { mDirection = RIGHT; moving = true; }

    void rotateLeft() {mAngularMovement = -50;}
    void rotateRight() {mAngularMovement = 50;}

    void loseLife() {lives -= 1; if (lives ==0) deactivate(); PlaySound(hurt);}
    int getLives() {return lives;}

    void resetMovement() { mMovement = { 0.0f, 0.0f }; moving = false; mAngularMovement = 0.0f;}
